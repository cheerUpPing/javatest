...
public class CommmUtil {
    ...
    public static Properties getProperties(String fileName) {
        ...
        InputStream in = CommmUtil.class.getClassLoader().getResourceAsStream(fileName);
        if (in == null) {
     ...
     * @return
     */
    public static byte[] mergeBytes(byte[] befores, byte[] lasts) {
        int beforeLength = (befores == null || befores.length == 0) ? 0 : befores.length;
        int lastLength = (lasts == null || lasts.length == 0) ? 0 : lasts.length;
