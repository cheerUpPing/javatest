...
var intervalTime; (function() {
    ...
    function b4() {
        ...
        if (isstudentDate) {
            $("#sf2").attr("disabled", "true");
            $("#sf2_label").addClass("color999");
            $("#query_ticket").removeClass().addClass("btn2s")
        }
    }
    function ah() {
        if (!isInitStationDiv) {
        ...
        if (co) {
            for (var cn = 0; cn < cm.length; cn++) {
                ...
                if (c(cp.queryLeftNewDTO.station_train_code)) {
                    showTicketPrice($("#SWZ_" + cp.queryLeftNewDTO.train_no)[0])
                }
            }
        }
    }
    ...
    function bV(cm) {
        if (cm && cm.length > 0) {
            if (DW_TRAINS && DW_TRAINS.length) {
                for (var cp = 0,
                         cr = cm.length; cp < cr; cp++) {
                    var cq = cm[cp].queryLeftNewDTO.station_train_code;
                    for (var cn = 0,
                             co = DW_TRAINS.length; cn < co; cn++) {
                        if (cq == DW_TRAINS[cn]) {
                            return true
                        }
                    }
    ...
    /**转换余票数据 cp：result cr:map**/
    function b2(cp, cr) {
        var co = [];
        for (var cn = 0; cn < cp.length; cn++) {
            var cs = [];
            var cm = cp[cn].split("|");//每一行result分割
            cs.secretStr = cm[0];
    ...
    function m(cy) {
        ...
        for (var cu = 0; cu < cs.length; cu++) {
            var cw = 0;
            if (cs[cu].toLowerCase() == "srrb" || cs[cu].toLowerCase() == "yyrw") {
                var cm = cz.queryLeftNewDTO["seat_types"];
                if (cs[cu].toLowerCase() == "srrb" && cm.indexOf("F") > -1) {
                    cw = cz.queryLeftNewDTO["rw_num"]
                }
    ...
    function bv() {
        ...
        if (typeof(TouLocal) != "undefined" && TouLocal.checkZByTargeElement("")) {
            ...
            cn = ce() + ($(window).height() / 2 - 232)
        } else {
            cm = $(window).width() / 2 - 200;
            cn = ce() + ($(window).height() / 2 - 205)
        }
    ...
    function aa(cn, cm) {
        ...
        $("#loginSubAsyn").click(function() {
            var co = aS();
            if (if_show_pass_code_login == "Y" && !verifyRandCode($("#randCode")[0], co)) {
                return
            }
            ...
            $("#loginForm").ajaxSubmit({
                ...
                success: function(cp) {
                    if (cp.data.status) {
                        if (cp.data.username != null) {
                            bt();
                            loginAsyn(cp.data.username);
                            if (cp.data.otherMsg != "") {
                                dhtmlx.alert({
                                    title: messages["message.error"],
                                    ok: messages["button.ok"],
                                    text: cp.data.otherMsg,
                                    type: "alert-error",
                                    callback: function() {
                                        if ("Y" == cp.data.notifysession) {
                                            dhtmlx.createWin({
                                                winId: "notifysession",
                        ...
                        $("#password").val("");
                        $("#randCode").val("");
                        if (cp.data.loginFail != null) {
                            if (cp.data.loginFail == "登录名不存在!") {
                                aB = false;
                                aP();
                                $("#username").removeClass("error").focus()
                            } else {
                                if (cp.data.loginFail == "验证码不正确！" && cp.data.loginFail != "登录名不存在!") {
                                    aP();
                            ...
                            if (if_show_pass_code_login == "Y") {
                                showError($("#randCode")[0], cp.data.loginFail)
                            } else {
                                login_errorMsg(cp.data.loginFail)
                            }
                        }
                    }
                }
